            ...
            if ($this->validator::validate($data, $rules)) {
                try {
                    ...
                    if (isset($_FILES['photo_recto']) && $_FILES['photo_recto']['error'] === UPLOAD_ERR_OK) {
                        ...
                        if ($photoRecto === false) {
                            $this->session->set('error', ['upload' => 'Erreur lors de l\'upload de la photo recto']);
                            $this->render('login/inscription.php');
                            return;
                        }
                    }
                    ...
                    if (isset($_FILES['photo_verso']) && $_FILES['photo_verso']['error'] === UPLOAD_ERR_OK) {
                        ...
                        if ($photoVerso === false) {
                            $this->session->set('error', ['upload' => 'Erreur lors de l\'upload de la photo verso']);
                            $this->render('login/inscription.php');
                            return;
                        }
                    }
                    ...
                    if ($result) {
                        ...
                        $this->session->set('error', ['inscription' => 'Erreur lors de l\'inscription']);
                    }
                } catch (\Exception $e) {
                    $this->session->set('error', ['exception' => $e->getMessage()]);
                }
            } else {
                $this->session->set('error', $this->validator::getError());
            }
        }
